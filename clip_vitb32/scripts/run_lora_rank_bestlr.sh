#!/bin/bash
# CLIP ViT-B/32 LoRA rank best-lr experiments (queue-based, multi-node safe)
#
# Sweeps over LoRA ranks with pre-determined best learning rates.
# Uses "train-test" split mode. lora_alpha = 2 * lora_r (fixed).
# Loop order: dataset -> lora_rank -> variant -> seed
#
# Usage:
#   bash scripts/run_lora_rank_bestlr.sh
#   RESET_QUEUE=1 bash scripts/run_lora_rank_bestlr.sh
#   GPU_IDS="0 1 2 3" bash scripts/run_lora_rank_bestlr.sh
...
LORA_RS=${LORA_RS:-"2 4 8 16 32 64 128 256 512"}
...
# Best LR per (variant, rank) — from sweep experiments
# Format: "2=lr1 4=lr2 8=lr3 ..." (key = rank)
BESTLR_LORA_ADAMW=${BESTLR_LORA_ADAMW:-"2=1e-2 4=7e-3 8=5e-3 16=3e-3 32=1e-3 64=1e-3 128=5e-4 256=3e-4 512=3e-4"}
BESTLR_LORA_MUON=${BESTLR_LORA_MUON:-"2=1e-2 4=5e-3 8=3e-3 16=3e-3 32=1e-3 64=1e-3 128=7e-4 256=3e-4 512=3e-4"}
BESTLR_LORA_MUON_PE=${BESTLR_LORA_MUON_PE:-""}

# Wandb
WANDB_PROJECT=${WANDB_PROJECT:-"clip_vit_lora_rank_bestlr"}
...
QUEUE_DIR=${QUEUE_DIR:-"$PWD/.queue_std_vitb32_lora_rank_bestlr"}
...
log_dir="logs/std_vitb32_lora_rank_bestlr"
mkdir -p "$log_dir"
...
# =============================================================================
# LR Lookup (by variant + rank)
# =============================================================================

# Lookup rank in "2=lr1 4=lr2 ..." spec string (strips optional "r" prefix)
_lookup_lr_in_spec() {
  local spec="$1" rank="${2#r}"
  for kv in $spec; do
    local k="${kv%%=*}" v="${kv#*=}"
    k="${k#r}"
    if [[ "$k" == "$rank" ]]; then echo "$v"; return 0; fi
  done
  return 1
}

get_best_lr_for_variant_rank() {
  local variant="$1" rank="$2" spec=""
  case "$variant" in
  ...
  if [[ -z "${spec// }" ]]; then
    echo "Missing BESTLR table for $variant. Set BESTLR_* env vars." >&2
    return 1
  fi

  _lookup_lr_in_spec "$spec" "$rank" \
    || { echo "Missing best lr for variant=$variant rank=$rank" >&2; return 1; }
}
...
# =============================================================================
# Generate Task List (dataset -> rank -> variant -> seed)
# =============================================================================
...
for ds in $DATASETS; do
  for lora_r in $LORA_RS; do
    for variant in $VARIANTS; do
      lr=$(get_best_lr_for_variant_rank "$variant" "$lora_r") || exit 1
      for seed in $SEEDS; do
        cmd=$(build_cmd "$variant" "$ds" "$lr" "$seed" "$lora_r") || exit 1
        task_list+=("$cmd")
...
echo "=============================================="
echo "CLIP ViT-B/32 Std LoRA Rank Best-LR Configuration"
echo "=============================================="
echo "Node:       ${NODE_NAME}"
echo "GPUs:       ${GPU_ID_ARR[*]}"
echo "Datasets:   ${DATASETS}"
echo "LoRA ranks: ${LORA_RS}"
echo "LoRA:       alpha=2*r, dropout=${LORA_DROPOUT}, visual_proj=${LORA_VISUAL_PROJECTION}, rslora=${USE_RSLORA}"
