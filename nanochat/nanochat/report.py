...
class Report:
    ...
    def generate(self):
        ...
        with open(report_file, "w") as out_file:
            ...
            # CORE first, rest alphabetical
            all_metrics = sorted(all_metrics, key=lambda x: (x != "CORE", x))
            # Fixed column widths
            stages = ["base"]
            metric_width = 15
            value_width = 8
...
def get_report(report_tag=None):
    # just for convenience, only rank 0 logs to report
    from nanochat.common import get_base_dir, get_dist_info
    ddp, ddp_rank, ddp_local_rank, ddp_world_size = get_dist_info()
    if ddp_rank == 0:
        # Allow the report dir suffix to be set via the REPORT_TAG env var or function arg
        tag = report_tag or os.environ.get("REPORT_TAG", "")
        report_dirname = f"report_{tag}" if tag else "report"
        report_dir = os.path.join(get_base_dir(), report_dirname)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tag", type=str, default=None, help="Report tag for directory name (e.g., d20_adam)")
    args = parser.parse_args()
    if args.command == "generate":
        get_report(args.tag).generate()
    elif args.command == "reset":
        get_report(args.tag).reset()
