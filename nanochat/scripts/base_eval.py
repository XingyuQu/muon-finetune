...
def main():
    ...
    parser.add_argument('--model_tag', type=str, default=None, help='Model tag for checkpoint directory name')
    parser.add_argument('--source', '-i', type=str, default='base', choices=['base', 'mid', 'sft'],
                        help='Source of the model: base|mid|sft (default: base)')
    parser.add_argument('--ckpt-dir', type=str, default=None,
                        help='Direct path to checkpoint directory (overrides --source and --model_tag)')
    ...
    if args.hf_path is not None:
        ...
        # load from a direct checkpoint directory path
        from nanochat.checkpoint_manager import find_last_step, build_model
        ...
        model_name = f"{ckpt_dir} (step {step})" # just for logging
        model_slug = f"base_eval_{step:06d}" # for the output csv file
        output_dir_override = ckpt_dir # save results in the checkpoint directory
    else:
        # load a local model from the file system
        model, tokenizer, meta = load_model(args.source, device, phase="eval", model_tag=args.model_tag)
        model_name = f"{args.source}_model (step {meta['step']})" # just for logging
    ...
    centered_results = {}
    if ddp_rank == 0:
        # if ckpt-dir mode, save results in the checkpoint directory; otherwise use base_eval/
        if 'output_dir_override' in dir():
            output_csv_path = os.path.join(output_dir_override, f"{model_slug}.csv")
        else:
            base_dir = get_base_dir()
